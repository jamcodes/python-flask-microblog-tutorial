from app import models, db, email
...
def before_request():
    ...
    # fields of the `g` object can be accessed in templates
...
def explore():
    page = request.args.get('page', 1, type=int)
    posts = models.Post.query.order_by(models.Post.timestamp.desc()).paginate(
        page, current_app.config['POSTS_PER_PAGE'], False)
...
def follow(username):
    ...
    db.session.commit()
    # flask-babel only supports the old format strings
...
def search():
    ...
    page = request.args.get('page', 1, type=int)
    posts, total = forms.Post.search(g.search_form.q.data, page,
